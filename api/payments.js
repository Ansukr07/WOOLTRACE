...
export default async function handler(req, res) {
  ...
  const { action = 'create', transactionId, amount, paymentId, method = 'UPI' } = req.body || {};
  if (action === 'create') {
    ...
    const intent = { id, transactionId, amount: Number(amount), method, reference, upiUri, gateway: process.env.PAYMENT_GATEWAY_PROVIDER || 'TEST_UPI', status: 'PENDING' };
  ...
  if (action === 'confirm') {
    ...
    const completed = { ...payment, status: 'PAID', paidAt: new Date().toISOString(), method };
