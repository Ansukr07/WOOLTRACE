...
const LANGUAGE_NAMES = { en: 'English', hi: 'Hindi', bn: 'Bengali', mr: 'Marathi', te: 'Telugu', ta: 'Tamil', gu: 'Gujarati', kn: 'Kannada', ml: 'Malayalam', pa: 'Punjabi', or: 'Odia' };
const renderMessage = (content = '') => content.split(/(\*\*[^*]+\*\*)/g).map((part, index) =>
...
export default function MarketChatbot() {
  ...
  const language = LANGUAGE_NAMES[user?.preferredLanguage || localStorage.getItem('khetsetu_language_guest')] || 'English';
  ...
  const send = async (event) => {
    ...
    if (language === 'English' && roleContent.quick[text]) { setMessages(current => [...current, { role: 'assistant', content: roleContent.quick[text] }]); return; }
    setLoading(true);
  ...
  const useSuggestion = (text) => { if (language === 'English' && roleContent.quick[text]) setMessages(current => [...current, { role: 'user', content: text }, { role: 'assistant', content: roleContent.quick[text] }]); else setInput(text); };
  if (!visible) return null;
  return <>{!open && <button className="market-chat-launcher" onClick={() => setOpen(true)} aria-label="Open market assistant"><MessageCircle size={20} /><span>Market Assistant</span></button>}{open && <section className="market-chat" aria-label="KhetSetu Market Assistant"><header><div className="market-chat-title"><div><strong>Market Assistant</strong><small>{user?.name ? `Guidance for ${user.name}` : 'Practical market guidance'}</small></div></div><button onClick={() => setOpen(false)} aria-label="Close chat"><X size={18} /></button></header><div className="market-chat-suggestions">{roleContent.suggestions.map(item => <button key={item} type="button" onClick={() => useSuggestion(item)}>{item}</button>)}</div><div className="market-chat-messages">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`market-chat-message ${message.role}`}>{renderMessage(message.content)}</div>)}{loading && <div className="market-chat-message assistant"><Loader2 className="market-chat-spin" size={15} /> Preparing an answer…</div>}</div><form onSubmit={send}><input value={input} onChange={event => setInput(event.target.value)} placeholder="Ask about prices, offers or payments" aria-label="Message" /><button type="submit" disabled={!input.trim() || loading} aria-label="Send message"><Send size={17} /></button></form></section>}</>;
}
